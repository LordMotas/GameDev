...
Game.input.Keyboard = function(){
	...
	function keyDown(event){
		...
		//Check to see if we already have this property so we don't overwrite the value that already exists.
		if(keyRepeat.hasOwnProperty(event.keyCode) === false){
			keyRepeat[event.keyCode] = false;
		}
	}

	//Called when the 'keyrelease' event is fired from the browser
