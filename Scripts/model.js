...
Game.model = (function(music, components){
	...
	that.initialize = function(){
		...
		enemyQueue[5].push(components.Enemy({
			...
			particleType: 2,
			isBoss : true,
			func : function(){music.resetMusic('Audio/mainBGM'); Game.bgm['Audio/bossBGM'].volume = 1.0; music.playMusic('Audio/bossBGM')}
		}));
	...
	function decision(e1, e2) {
		if((e1.isPlayer && e2.isBullet) || (e2.isPlayer && e1.isBullet) || (e1.isGraze && e2.isBullet) || (e1.isBullet && e2.isGraze))
			return true;
		else
			return false;
	}
		...
		for(enemyBullet = 0; enemyBullet < enemyBullets.length; enemyBullet++){
			...
			if (other) {
				if(decision(enemyBullets[enemyBullet], other)){
					if(other.isGraze && enemyBullets[enemyBullet].hasBeenGrazed){
						enemyBullets[enemyBullet].hasBeenGrazed = false;
