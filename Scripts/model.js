...
Game.model = (function(components){
	...
	function addMoreCircles(howManyCircles){
		...
		while (addedCircles < howManyCircles){
			...
			//Make sure this circle doesn't intersect/overlap with any of our existing circles.
			intersectsAny = false;
			for(circle = 0; circle < circles.length; circle += 1){
	...
	//Perform an elastic collision between the two circles to determine their new direction vectors
	...
	function notifyResize(){
		...
		textObjects.height = Game.renderer.core.measureTextHeight(textObjects);
		textTests.height = Game.renderer.core.measureTextHeight(textTests);
		textCriteria.height = Game.renderer.core.measureTextHeight(textCriteria);

		textTests.pos.y = textObjects.pos.y + textObjects.height + 0.01;
		textCriteria.pos.y = textTests.pos.y + textTests.height + 0.01;
	}

	//This function initializes the quad-tree Game model.  Only thing it
	//does right now is to add the circles to the model
	that.initialize = function(howManyCircles){

		Game.renderer.core.notifyResize(notifyResize);
					...
					for(test = circle; test < circles.length; test += 1){
						...
						if(circle !== test){
							...
							if(circles[circle].intersects(circles[test])){
								...
								//Bounce the circles...this also moves them so they no longer intersect
